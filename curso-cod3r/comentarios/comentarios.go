...
/*
 ...
 - Para executar arquivos pelo terminal = go run pasta/*.go
...
func main() {
 ...
 - flags de impressão: EX: fmt.Println("%d %g %t %s %v %.2f" 1, 81632.64, true, "AbCd", cidade, 64.32168)
	retorna um valor do tipo int e uint: %d
	retorna um valor do tipo uint: %#x somente se usado com: %#v
	retorna um valor do tipo float: %g
	retorna um valor do tipo boolean: %t
	retorna um valor do tipo string: %s
	retorna um valor de tipo pointer e channel: %p
 ...
 - _ = blank indentifier é uma variável sem valor que pode ser utilizada sem problemas, ex:
 _, y, _ := coord(p)  // Retornaria 3 valores mas vai retornar somento o valor de y sem erros
